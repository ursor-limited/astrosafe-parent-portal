...
async function fetchAndRenameSitemap(sitemapUrl: string): Promise<ISitemap[]> {
  try {
    ...
    urlArray.push({'url': 'https://www.astrosafe.co/tools/math-worksheet-generator'})
    urlArray.push({'url': 'https://www.astrosafe.co/tools/multiplication-chart'})
    for (let i = 0; i <= 30; i++) {
      urlArray.push({'url': 'https://www.astrosafe.co/tools/multiplication-chart/'+i+'-times-table-worksheet'})
    }
