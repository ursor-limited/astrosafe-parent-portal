...
export default function InsuranceApplicationForm(props: {
  ...
  useEffect(
    () =>
      setCurrentStepAnswers(
        committedAnswers?.find((step) => step.stepId === STEPS[stepIndex].id)
          ?.answers || []
      ),
    [stepIndex]
  );
  ...
  const [committedAnswers, setCommittedAnswers] = useLocalStorage<
    { stepId: string; answers: IAWFormInputAnswer[] }[] | undefined
  >("committedAnswers", undefined);
  // useEffect(
  //   () => committedAnswers && setCurrentStepAnswers(committedAnswers),
  //   [committedAnswers]
  // );
      ...
      setCanProceed(
        STEPS[stepIndex].sections
          ...
          .every(
            (input) =>
              input.optional ||
              currentStepAnswers.find((a) => a.inputId === input?.id)?.value
          )
      ),
    [currentStepAnswers, stepIndex]
  );
    ...
    setCommittedAnswers(
      committedAnswers?.find((step) => step.stepId === stepId)
        ? committedAnswers?.map((step) =>
            step.stepId === stepId
              ? { stepId: stepId, answers: currentStepAnswers }
              : step
          )
        : [
            ...(committedAnswers ?? []),
