...
export default function UserLessonPage(props: { userId: string }) {
  ...
  const [sortedLessonList, setSortedLessonList] = useState<ILesson[] | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    ApiController.getUserLessons(props.userId).then((a: ILesson[]) => {
      console.log(props.userId, "opopopopo");
        ...
        <th></th>
          ...
          <tr>
            <td>
              {
                <Button
                  onClick={() => sortActivity("title")}
                  sortingDirection={sortField === "title" ? sortBy : null}
                >
                  Title
                </Button>
              }
            ...
            <td>
              {
                <Button
                  onClick={() => sortActivity("contentAmount")}
                  sortingDirection={
                    sortField === "contentAmount" ? sortBy : null
                  }
                >
                  Ttl. Cnt.
                </Button>
        ...
        <tbody>
          {sortedLessonList.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                {
                  <a href={`https://dev.astrosafe.co/lesson/${item.id}`}>
                    Go to lesson
                  </a>
                }
              </td>
